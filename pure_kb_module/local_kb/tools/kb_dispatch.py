...
def _score_keywords(
    ...
    weights = weights or {}
    negatives = negative_keywords or []
    score = 0
    matched: list[str] = []
    for keyword in keywords:
        token = _normalize(keyword)
        if token and token in normalized:
            score += weights.get(keyword, DEFAULT_KEYWORD_WEIGHT)
...
def _pick_best(text: str, routes: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for name, spec in routes.items():
        ...
        score, matched = _score_keywords(
            text,
            spec.get("keywords", []),
            weights=spec.get("keyword_weights"),
...
def _detect_ambiguity(
    ...
    if ratio >= (1.0 - threshold):
        return {
            "ambiguous": True,
            "top_domain": ranked[0]["name"],
            "second_domain": ranked[1]["name"],
            "top_score": top_score,
            "second_score": second_score,
            "score_ratio": round(ratio, 2),
            "message": (
                f"'{ranked[0]['name']}'({top_score}) 与 '{ranked[1]['name']}'({second_score}) "
                f"得分接近(ratio={ratio:.2f})，建议用 keyword_weights 或 negative_keywords 消除歧义。"
            ),
        }
    return None


def _detect_keyword_conflicts(domains: dict[str, Any]) -> list[dict[str, Any]]:
    domain_names = [name for name, spec in domains.items() if isinstance(spec, dict)]
    conflicts: list[dict[str, Any]] = []
    for i in range(len(domain_names)):
        for j in range(i + 1, len(domain_names)):
            name_a, name_b = domain_names[i], domain_names[j]
            spec_a, spec_b = domains[name_a], domains[name_b]
            keywords_a = {kw: _normalize(kw) for kw in spec_a.get("keywords", [])}
            keywords_b = {kw: _normalize(kw) for kw in spec_b.get("keywords", [])}
            norm_a = set(keywords_a.values())
            norm_b = set(keywords_b.values())
            shared_norm = norm_a & norm_b
            if not shared_norm:
                continue
            overlapping = sorted(
                kw for kw, nkw in keywords_a.items() if nkw in shared_norm
            )
            conflicts.append({
                "domain_a": name_a,
                "domain_b": name_b,
                ...
                "message": (
                    f"领域 '{name_a}' 与 '{name_b}' 共享关键词: "
                    f"{', '.join(overlapping)}，建议拆分或加 negative_keywords。"
                ),
            })
    conflicts.sort(key=lambda c: -len(c["overlapping_keywords"]))
    return conflicts
...
def _detect_entry_contention(manifest: dict[str, Any]) -> list[dict[str, Any]]:
    domain_routing = manifest.get("domain_routing", {})
    all_entries: list[dict[str, Any]] = []
    for domain_name, spec in domain_routing.items():
        if not isinstance(spec, dict):
            continue
        for file_path in spec.get("read", []):
            path = _resolve_read_path(file_path)
            if not path.exists() or path.suffix.lower() != ".md":
                continue
            for entry in _load_structured_entries(path):
                all_entries.append({**entry, "_domain": domain_name})
    ...
    for i in range(len(all_entries)):
        for j in range(i + 1, len(all_entries)):
            ...
            pair_key = (a["file"], b["file"]) if a["file"] < b["file"] else (b["file"], a["file"])
            ...
            symptom_a = str(a.get("symptom", ""))
            symptom_b = str(b.get("symptom", ""))
            title_overlap = _token_overlap(_tokenize_text(title_a), _tokenize_text(title_b)) > 0
            symptom_overlap = _token_overlap(_tokenize_text(symptom_a), _tokenize_text(symptom_b)) > 0

            if not title_overlap and not symptom_overlap:
                continue

            actions_a = set(_ensure_list(a.get("recommended_action")))
            actions_b = set(_ensure_list(b.get("recommended_action")))
            ...
            if title_overlap:
                reason.append("title")
            if symptom_overlap:
                reason.append("symptom")

            contentions.append({
                "entry_a": {
                    "domain": a.get("_domain", ""),
                    "file": a["file"],
                    "title": title_a,
                },
                "entry_b": {
                    "domain": b.get("_domain", ""),
                    "file": b["file"],
                    "title": title_b,
                },
                "similarity_reason": "+".join(reason),
                "actions_a": sorted(actions_a),
                "actions_b": sorted(actions_b),
                "message": (
                    f"'{title_a}' 与 '{title_b}' 在 {'+'.join(reason)} 上相似，"
                    f"但推荐动作不同，可能存在知识矛盾。"
                ),
            })
    ...
    contentions.sort(
        key=lambda c: (
            0 if c["entry_a"]["domain"] != c["entry_b"]["domain"] else 1,
            -(len(c["actions_a"]) + len(c["actions_b"])),
        )
    )
    return contentions
...
def _build_best_entry_payload(manifest: dict[str, Any], text: str, max_entries: int = 3) -> dict[str, Any]:
    ...
    top_sub_domain = None
    if top_domain:
        domain_name = top_domain["name"]
        domain_spec = manifest.get("domain_routing", {}).get(domain_name, {})
        sub_domains = _pick_best(text, domain_spec.get("sub_domains", {}))
        if sub_domains:
            top_sub_domain = sub_domains[0]
            for kw in top_sub_domain.get("matched", []):
                if kw not in (top_domain.get("matched") or []):
                    top_domain["matched"].append(kw)
                    top_domain["score"] += top_sub_domain["score"]

    preferred_files = set(top_domain.get("read", [])) if top_domain else set()
    source_files: list[str] = []
    if top_sub_domain:
        for item in top_sub_domain.get("read", []):
    ...
    return {
        ...
        "fallback": fallback,
        "ambiguity_warning": _detect_ambiguity(domains),
    }
...
def _creation_proposal(
    ...
    override_type = creation_type

    if override_type == "route-optimization":
        best_payload = _build_best_entry_payload(manifest, text)
        ambiguity = best_payload.get("ambiguity_warning")
        top_domain = best_payload.get("domain")
        top_sub = best_payload.get("sub_domain")
        domain_routing = manifest.get("domain_routing", {})
        all_conflicts = _detect_keyword_conflicts(domain_routing)
        relevant_conflicts = [
            c for c in all_conflicts
            if c["domain_a"] == (top_domain.get("name") if top_domain else "")
            or c["domain_b"] == (top_domain.get("name") if top_domain else "")
        ]
        suggestions: list[str] = []
        if ambiguity:
            suggestions.append(
                f"'{ambiguity['top_domain']}' 与 '{ambiguity['second_domain']}' "
                f"得分接近 (ratio={ambiguity['score_ratio']})，建议增加 keyword_weights "
                f"区分或为其中一个域添加 negative_keywords。"
            )
        if relevant_conflicts:
            for c in relevant_conflicts:
                suggestions.append(
                    f"'{c['domain_a']}' 与 '{c['domain_b']}' 共享关键词 "
                    f"({', '.join(c['overlapping_keywords'])})，建议拆分或细化。"
                )
        if top_domain and not top_sub:
            domain_spec = domain_routing.get(top_domain["name"], {})
            if not domain_spec.get("sub_domains"):
                suggestions.append(
                    f"领域 '{top_domain['name']}' 尚无 sub_domains，"
                    f"建议按当前高频主题拆分子域以增加路由深度。"
                )
        if not suggestions:
            suggestions.append("当前路由未检测到明显问题，建议人工审阅 manifest 后再决定。")

        return {
            "requires_user_confirmation": True,
            "creation_type": "route-optimization",
            "reason": "Governance feedback 指出当前路由层需要优化。",
            "question": "是否要根据以下建议调整 kb_manifest.yaml 的路由规则？",
            "target_file": "local_kb/kb_manifest.yaml",
            "suggestions": suggestions,
            "ambiguity_warning": ambiguity,
            "keyword_conflicts": relevant_conflicts,
        }
...
def cmd_route(args: argparse.Namespace) -> int:
    ...
    payload = {
        ...
        "sub_domain": best_entry_payload.get("sub_domain"),
        ...
        "ambiguity_warning": best_entry_payload.get("ambiguity_warning"),
        "keyword_conflicts": [
            c for c in _detect_keyword_conflicts(manifest.get("domain_routing", {}))
            if c["domain_a"] in {d["name"] for d in domains[: args.top]}
            or c["domain_b"] in {d["name"] for d in domains[: args.top]}
        ],
        "creation_proposal": (
            None
            if domains
...
def cmd_propose_create(args: argparse.Namespace) -> int:
    ...
    domains = _pick_best(args.text, manifest.get("domain_routing", {}))
    top_domain = domains[0] if domains else None
    creation_type = getattr(args, "creation_type", None)
    payload = _creation_proposal(
        manifest, args.text,
        top_domain if args.force_note else None,
        domains,
        creation_type=creation_type,
    )
...
def cmd_check_conflicts(args: argparse.Namespace) -> int:
    manifest = _load_manifest()
    domain_routing = manifest.get("domain_routing", {})
    keyword_conflicts = _detect_keyword_conflicts(domain_routing)
    entry_contentions = (
        _detect_entry_contention(manifest) if not args.skip_entries else []
    )
    payload = {
        "keyword_conflicts": keyword_conflicts,
        "entry_contentions": entry_contentions,
        "summary": {
            "total_keyword_conflicts": len(keyword_conflicts),
            "total_entry_contentions": len(entry_contentions),
        },
    }
    _emit(payload, args.format)
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    propose.add_argument("--creation-type", choices=["route-optimization"], help="Force a specific creation proposal type.")
    ...
    check_conflicts = subparsers.add_parser(
        "check-conflicts",
        help="Detect keyword conflicts between domains and contentions between structured entries.",
    )
    check_conflicts.add_argument(
        "--skip-entries",
        action="store_true",
        help="Skip structured entry contention detection (keyword conflicts only).",
    )
    check_conflicts.add_argument(
        "--format",
    ...
    check_conflicts.set_defaults(func=cmd_check_conflicts)
